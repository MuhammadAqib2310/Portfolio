...
const skills = [
  ...
  { name: "Full Stack Dev",        pct: 92, color: "#00E5FF" },
...
export default function About() {
  ...
  return (
    <section id="about" ref={ref} className="section-pad relative overflow-hidden" style={{ background:"linear-gradient(180deg, #0B0B18 0%, #0E0C22 100%)" }}>
      {/* BG glows */}
      <div className="absolute pointer-events-none rounded-full"
        style={{ width:500, height:500, top:"-10%", right:"-10%",
          background:"radial-gradient(circle,rgba(0,229,255,0.07),transparent 65%)", filter:"blur(80px)" }} />
      <div className="absolute pointer-events-none rounded-full"
        style={{ width:400, height:400, bottom:0, left:"-8%",
          background:"radial-gradient(circle,rgba(108,99,255,0.07),transparent 65%)", filter:"blur(70px)" }} />
        ...
        <div className="grid lg:grid-cols-2 gap-12 xl:gap-20 items-start">
          ...
          <motion.div variants={fadeLeft(0.1)} initial="hidden" animate={inView?"show":{}}>
            ...
            <motion.a href="/resume.pdf" download
              whileHover={{ scale:1.03, boxShadow:"0 0 32px rgba(108,99,255,0.4)" }}
              whileTap={{ scale:.98 }}
          ...
          <motion.div variants={fadeRight(0.2)} initial="hidden" animate={inView?"show":{}}
            ...
            <h3 className="text-base font-bold mb-8 flex items-center gap-2" style={{ color:"#fff" }}>
              <span className="w-1 h-6 rounded-full" style={{ background:"linear-gradient(#6C63FF,#00E5FF)" }} />
              Core Competencies
            </h3>
