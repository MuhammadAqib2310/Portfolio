...
/* ─────────────────────────────────────────────────────────
   ...
   Props:
     ...
     accent:  primary color hex  (default #2563EB)
     cyan:    secondary color hex (default #38BDF8)
...
function ProcessConnectors({ accent, cyan }: { accent: string; cyan: string }) {
  return (
    ...
    >
      ...
      {[0, 1, 2].map(idx => (
        <circle key={idx} r="4" fill={idx === 0 ? accent : idx === 1 ? cyan : "#6366F1"} filter="url(#processGlow)">
  ...
  return (
    <div style={{ position: "absolute", inset: 0, pointerEvents: "none", overflow: "hidden", zIndex: 0 }}>
      ...
      <div style={{
        ...
        background: `linear-gradient(135deg, ${accent}22 0%, ${cyan}16 35%, #6366F114 65%, transparent 80%)`,
        filter: "blur(70px)",
  ...
  return (
    <div style={{ position: "absolute", inset: 0, pointerEvents: "none", zIndex: 0, overflow: "hidden" }}>
      {[
        ...
        { top: "10%", left: "70%", w: 340, h: 300, color: "#6366F1", delay: 6, dur: 22 },
      ].map((b, i) => (
        <div key={i} style={{
          position: "absolute", borderRadius: "50%", pointerEvents: "none",
...
function FloatingGlass({ accent, cyan }: { accent: string; cyan: string }) {
  const shapes = [
    ...
    { size: 65,  top: "15%", left: "88%", delay: 6,   dur: 14, color: "#6366F1" },
    { size: 45,  top: "80%", left: "85%", delay: 2,   dur: 18, color: accent },
    { size: 100, top: "45%", left: "92%", delay: 8,   dur: 20, color: cyan },
    { size: 70,  top: "55%", left: "1%",  delay: 4,   dur: 15, color: "#6366F1" },
  ];
  return (
    <div style={{ position: "absolute", inset: 0, pointerEvents: "none", zIndex: 0, overflow: "hidden" }}>
      {shapes.map((s, i) => (
