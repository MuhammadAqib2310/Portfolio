...
export default function Hero() {
  ...
  return (
    <section id="hero" className="relative min-h-screen flex items-center overflow-hidden"
      style={{ background: "linear-gradient(135deg, #0D0A0E 0%, #13101A 50%, #0D0A0E 100%)" }}>
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute rounded-full anim-aurora"
          style={{ width: "min(900px,120vw)", height: "min(900px,120vw)", top: "-30%", left: "-20%",
            background: "radial-gradient(circle, rgba(232,121,160,0.28) 0%, transparent 65%)", filter: "blur(70px)" }} />
        <div className="absolute rounded-full"
          style={{ width: "min(700px,90vw)", height: "min(700px,90vw)", bottom: "-20%", right: "-15%",
            background: "radial-gradient(circle, rgba(252,165,192,0.22) 0%, transparent 65%)", filter: "blur(70px)",
            animation: "aurora 22s ease-in-out infinite reverse" }} />
        <div className="absolute rounded-full"
          style={{ width: "min(500px,60vw)", height: "min(500px,60vw)", top: "30%", left: "45%",
            background: "radial-gradient(circle, rgba(192,132,160,0.15) 0%, transparent 65%)", filter: "blur(60px)",
        ...
        <div className="absolute rounded-full"
          style={{ width: "min(400px,50vw)", height: "min(400px,50vw)", bottom: "10%", left: "20%",
            background: "radial-gradient(circle, rgba(192,132,160,0.1) 0%, transparent 65%)", filter: "blur(80px)",
            animation: "aurora 17s ease-in-out infinite 3s" }} />
        {/* Soft radial gradient center vignette for depth */}
        <div className="absolute inset-0"
          style={{ background: "radial-gradient(ellipse 100% 80% at 50% 40%, rgba(232,121,160,0.04) 0%, transparent 70%)" }} />
      </div>

      {/* Dot grid */}
      <div className="absolute inset-0 pointer-events-none"
        style={{ backgroundImage: "radial-gradient(rgba(252,165,192,0.14) 1px, transparent 1px)",
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-16 lg:gap-10">
          ...
          <motion.div variants={stagger} initial="hidden" animate="show"
            ...
            <motion.div variants={item}>
              <div className="inline-flex items-center gap-2.5 mb-8 font-semibold"
                style={{ padding: "0.45rem 1rem", borderRadius: "999px", fontSize: "0.78rem",
                  background: "rgba(232,121,160,0.15)", border: "1px solid rgba(232,121,160,0.35)",
            ...
            <motion.div variants={item}
              ...
              <span className="anim-pulse-glow" style={{ color: "#E879A0", fontSize: "1.1em", marginLeft: "1px" }}>|</span>
            </motion.div>
            ...
            <motion.div variants={item} className="flex flex-wrap justify-center lg:justify-start gap-3">
              <MagneticButton
                ...
                style={{ padding: "0.85rem 2rem",
                  ...
                  boxShadow: "0 4px 28px rgba(232,121,160,0.45)" }}>
                View Projects →
              </MagneticButton>
              <MagneticButton
              ...
              <MagneticButton href="/resume.pdf" download
                ...
                style={{ padding: "0.85rem 2rem",
                  border: "1px solid rgba(252,165,192,0.35)", color: "#FCA5C0",
                  background: "rgba(252,165,192,0.07)" }}>
                ↓ Resume
              </MagneticButton>
            </motion.div>
          </motion.div>
          ...
          <motion.div
            ...
            <div className="absolute inset-0 pointer-events-none rounded-full"
              style={{ background: "radial-gradient(circle, rgba(232,121,160,0.4) 0%, rgba(252,165,192,0.18) 45%, transparent 70%)",
            ...
            <div className="absolute anim-spin-slow pointer-events-none"
              style={{ inset: "-12px", borderRadius: "50%",
                background: "conic-gradient(from 0deg, #E879A0 0%, #FCA5C0 33%, #C084A0 66%, transparent 80%, #E879A0 100%)",
                WebkitMask: "radial-gradient(farthest-side, transparent calc(100% - 3px), black calc(100% - 3px))",
                mask: "radial-gradient(farthest-side, transparent calc(100% - 3px), black calc(100% - 3px))" }} />

            {/* Static ring */}
            <div className="absolute pointer-events-none"
              style={{ inset: "-6px", borderRadius: "50%", border: "1px solid rgba(232,121,160,0.25)" }} />

            {/* Photo */}
            <div className="relative rounded-full overflow-hidden"
              style={{ width: "clamp(220px, 28vw, 320px)", height: "clamp(220px, 28vw, 320px)",
                border: "3px solid rgba(232,121,160,0.35)",
                boxShadow: "0 0 0 1px rgba(252,165,192,0.12), inset 0 0 40px rgba(232,121,160,0.08)" }}>
              {imgError ? (
                <div className="absolute inset-0 flex items-center justify-center font-black select-none"
                  style={{ fontSize: "clamp(3rem,8vw,5rem)",
                    background: "linear-gradient(135deg, rgba(232,121,160,0.2), rgba(252,165,192,0.1))", color: "#E879A0" }}>
                  MA
                </div>
              ) : (
            ...
            <motion.div animate={{ y: [-8, 8, -8] }} transition={{ duration: 4.5, repeat: Infinity, ease: "easeInOut" }}
              ...
              style={{ left: 0, top: "26%", padding: "0.5rem 0.875rem", borderRadius: "0.875rem",
                background: "rgba(232,121,160,0.2)", border: "1px solid rgba(232,121,160,0.5)",
            ...
            <motion.div animate={{ y: [8, -8, 8] }} transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }}
              ...
              style={{ right: 0, bottom: "26%", padding: "0.5rem 0.875rem", borderRadius: "0.875rem",
                background: "rgba(252,165,192,0.15)", border: "1px solid rgba(252,165,192,0.4)",
                color: "#FCA5C0", backdropFilter: "blur(16px)", zIndex: 10,
                boxShadow: "0 4px 20px rgba(252,165,192,0.18)" }}>
              ⚡ Full Stack
            </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.5 }}
        ...
        <motion.div animate={{ y: [0, 10, 0] }} transition={{ duration: 1.6, repeat: Infinity }}
          style={{ width: "1px", height: "36px",
            background: "linear-gradient(to bottom, #E879A0, transparent)", borderRadius: "1px" }} />
      </motion.div>
    </section>
  );
}
