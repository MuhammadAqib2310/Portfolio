        ...
        >
          ...
          <div className="absolute pointer-events-none rounded-full anim-aurora"
            style={{ width: 500, height: 500, top: "5%", left: "5%",
              background: "radial-gradient(circle, rgba(201,162,39,0.12), transparent 65%)", filter: "blur(70px)" }} />
          <div className="absolute pointer-events-none rounded-full"
            style={{ width: 400, height: 400, bottom: "5%", right: "5%",
              background: "radial-gradient(circle, rgba(30,58,95,0.08), transparent 65%)", filter: "blur(70px)",
          ...
          <motion.div
            ...
            className="text-center relative z-10" style={{ marginBottom: "3rem" }}>
            <motion.div
              animate={{ rotate: [0, 6, -6, 0], boxShadow: ["0 8px 32px rgba(30,58,95,0.2)", "0 8px 48px rgba(201,162,39,0.4)", "0 8px 32px rgba(30,58,95,0.2)"] }}
              transition={{ duration: 2.5, repeat: Infinity, ease: "easeInOut" }}
              className="w-20 h-20 rounded-3xl flex items-center justify-center text-2xl font-black text-white mx-auto mb-6"
              style={{ background: "linear-gradient(135deg, #1E3A5F, #2563EB)", letterSpacing: "-0.02em", fontSize: "1.1rem" }}
            >
              MA
            </motion.div>

            <div style={{ fontSize: "clamp(2.5rem, 7vw, 3.5rem)", fontWeight: 900,
              letterSpacing: "-0.04em", fontFamily: "var(--font-space-grotesk, sans-serif)", fontVariantNumeric: "tabular-nums" }}>
              <span style={{
                background: revealed
                  ? "linear-gradient(135deg, #1E3A5F 0%, #C9A227 100%)"
                  : "linear-gradient(135deg, #C9A227 0%, #1E3A5F 100%)",
            ...
            <AnimatePresence mode="wait">
              <motion.p key={wordIdx}
                ...
                style={{ fontSize: "0.7rem", letterSpacing: "0.28em", fontWeight: 700,
                  textTransform: "uppercase", color: "#C9A227", marginTop: "0.625rem" }}>
          ...
          <div style={{ width: "clamp(180px, 32vw, 260px)", position: "relative", zIndex: 10 }}>
            <div className="relative rounded-full overflow-hidden mb-3"
              style={{ height: "2px", background: "rgba(30,58,95,0.1)" }}>
              <motion.div className="absolute top-0 left-0 h-full rounded-full"
                style={{ width: `${Math.min(progress, 100)}%`,
                  background: "linear-gradient(90deg, #1E3A5F, #C9A227, #2563EB)",
                  boxShadow: "0 0 8px rgba(201,162,39,0.5)", transition: "width 0.08s linear" }} />
            ...
            <div className="flex justify-between items-center">
              <span style={{ fontSize: "0.62rem", color: "#94A3B8", fontWeight: 600, letterSpacing: "0.12em" }}>LOADING</span>
              <motion.span style={{ fontSize: "0.72rem", fontWeight: 900, fontVariantNumeric: "tabular-nums" }}
                animate={{ color: progress === 100 ? "#C9A227" : "#1E3A5F" }}>
                {Math.min(Math.round(progress), 100)}%
              </motion.span>
            </div>
          </div>

          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 0.4 }} transition={{ delay: 0.8 }}
            style={{ position: "absolute", bottom: "2rem", fontSize: "0.62rem",
              color: "#94A3B8", letterSpacing: "0.2em", fontWeight: 600 }}>
